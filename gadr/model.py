...
class Model(object):
    def __init__(self, args, logger, optimizer, fastnet, device, start_iter=0, start_epoch=0,
                 best_epe=None, best_epoch=None):
        self.args = args
        self.logger = logger
        self.optimizer = optimizer
        self.fastnet = fastnet
        self.device = device
        ...
        if not args.evaluate_only:
            self.train_writer = SummaryWriter(self.args.checkpoints_dir)
    ...
    def train(self, train_loader):
        ...
        if args.freeze_bn:
            ...
            self.fastnet.apply(set_bn_eval)
        ...
        for i, sample in enumerate(train_loader):
            ...
            pred_disp_pyramid = self.fastnet(left, right)  # H/4, H/2, H
        ...
        if args.no_validate:
            utils.save_checkpoint(args.checkpoints_dir, self.optimizer, self.fastnet,
            ...
            if self.epoch % args.save_ckpt_freq == 0:
                model_dir = os.path.join(args.checkpoints_dir, 'models')
                utils.check_path(model_dir)
                utils.save_checkpoint(model_dir, self.optimizer, self.fastnet,
                                      epoch=self.epoch, num_iter=self.num_iter,
                                      epe=-1, best_epe=self.best_epe,
                                      best_epoch=self.best_epoch,
    ...
    def validate(self, val_loader):
        ...
        if args.evaluate_only is True:
            if args.pretrained_aanet is not None:
                ...
                pretrained_aanet = os.path.join(args.checkpoints_dir, model_name)
            ...
            utils.load_pretrained_net(self.fastnet, pretrained_aanet, no_strict=True)
        ...
        val_file = os.path.join(args.checkpoints_dir, 'val_results.txt')
        ...
        for i, sample in enumerate(val_loader):
            ...
            with torch.no_grad():
                pred_disp = self.fastnet(left, right)[-1]  # [B, H, W]
