...
def disp_warp(img, disp, padding_mode='border'):
    ...
    warped_img = F.grid_sample(img, sample_grid, mode='bilinear', padding_mode=padding_mode,align_corners=True) #[B,3,H,W]

    mask = torch.ones_like(img)#[B,3,H,W]
    valid_mask = F.grid_sample(mask, sample_grid, mode='bilinear', padding_mode='zeros',align_corners=True)#[B,3,H,W]
